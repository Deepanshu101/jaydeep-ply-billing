...
const defaultTerms =
  "1. Goods once sold will not be taken back without prior approval.\n2. Taxes are charged as applicable.\n3. Delivery, unloading, and installation, if any, will be billed separately.\n4. Payment as per agreed terms.";
...
export function InvoiceForm({
  ...
  const totals = useMemo(
    () => calculateTotals(items, gstPercent, discountValue, discountType),
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <section className="overflow-hidden rounded-md border border-[#d8dfd7] bg-white">
        <div className="flex flex-col gap-3 border-b border-[#d8dfd7] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-bold">Inventory invoice rows</h2>
            <p className="text-sm text-[#5d6b60]">Names should match Tally stock items before pushing.</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[920px] w-full text-sm">
            <thead className="bg-[#eef3ee] text-left">
              <tr>
                {["Stock item", "Specification", "Qty", "Unit", "Rate / Per", "Amount", ""].map((heading) => (
      ...
      <section className="grid gap-4 rounded-md border border-[#d8dfd7] bg-white p-4 lg:grid-cols-[1fr_360px]">
        <label>
          <span className="text-sm font-semibold">Terms and narration</span>
          <textarea
